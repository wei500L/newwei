...
| shadow | `/api/healthz/live`、`/api/user-settings/ui/onboarding`（仅 GET） | NestJS 响应 + Go 实现异步差分 |
| canary | （无） | 待鉴权基础设施接入的分流组件（见下） |
| go | `/__go/healthz` | Go 原生（网关自省） |

### user-settings onboarding shadow（第二个迁移单元，Go-批2A）

- **范围**：仅 `GET /api/user-settings/ui/onboarding`。PUT 及其他五个
  user-settings GET 全部保持 legacy；不迁移写入路径。
- **行为**：客户端响应完全来自 NestJS；Go 在旁路真实读取 MySQL
  `UserSetting` 表（`orgId+userId+ui:onboarding:settings:v1` 三条件参数化
  查询），并与 NestJS 响应差分（normalization 契约逐字段对齐）。
...
- **失败非阻断**：MySQL 未配置/不可达/超时、JSON 异常、限流/并发预算
  ...
  NestJS 原响应。
- **不能进入 canary/go**：在 Go 完成 Auth/RBAC（迁移序 5）前，该端点
  保持 shadow；路由表被误改为 ModeCanary/ModeGo 时
  `cmd/api/main_test.go` 的状态契约测试会失败。
- **回滚**：`internal/legacyproxy/proxy.go` 中该路由单条改回
  `ModeLegacy`——纯代码变更，无数据耦合。
...
```bash
pnpm --filter @modular/api-go test    # 网关行为测试（四态/代理透传/go 路由/502/trace/shadow 预算/canary 哈希/onboarding 身份门禁）
pnpm --filter @modular/api-go lint    # go vet
pnpm --filter @modular/api-go build   # go build
```

MySQL 集成测试（Go-批2A，本机禁跑——远端 CI 的
`api-go-user-settings-integration` job 使用固定版本 MySQL service 执行）：
...
- shadow 只对 GET/HEAD/OPTIONS 差分——写请求禁止双发（双层强制：legacyproxy + shadow runner；onboarding shadow 单元进一步收窄为仅 GET）

...
import { DEFAULT_ROLES, CORE_PERMISSIONS } from "@modular/config";
  ...
  async createOrg(actorId: string, input: CreateOrgInput): Promise<OrgListItem> {
    ...
    const org = await this.prisma.$transaction(async (tx) => {
      ...
      const permissionRows = await tx.permission.findMany({
        where: { name: { in: CORE_PERMISSIONS as unknown as string[] } }
      });
      ...
      for (const roleDef of DEFAULT_ROLES) {
        ...
        const rolePermissions = roleDef.permissions
          .map((permission) => permissionByName.get(permission))
          .filter(Boolean)
          .map((permissionId) => ({
            roleId: role.id,
        ...
        if (rolePermissions.length !== roleDef.permissions.length) {
          throw new NotFoundException("Missing permissions during role bootstrap");
        }

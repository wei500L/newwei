...
export class QualityGateway implements OnGatewayConnection, OnGatewayDisconnect, OnModuleInit, OnModuleDestroy {
  @WebSocketServer()
  server!: Server;

  private readonly logger = createLogger({ name: "quality-gateway" });
  ...
  async handleConnection(client: Socket) {
    ...
    try {
      ...
      if (!profile.permissions.includes("settings.manage")) {
      ...
      client.emit("quality:connected", { orgId: profile.orgId, userId: profile.id });
      this.logger.info(
  ...
  private broadcast(orgId: string, payload: QualityLiveEventPayload) {
    if (!this.server) {
      return;
    }
    this.server.to(this.sessions.orgRoom(orgId)).emit("quality:event", { orgId, ...payload });
  }

...
describe("RbacService", () => {
  ...
  beforeEach(() => {
    ...
    userAdminServiceMock.invalidateUsersWithRole = jest
      .fn()
  ...
  it("resets assigned roles to a single role during compatibility assignment", async () => {
    ...
    prismaMock.membership.upsert = jest
      .fn()
    ...
    expect(prismaMock.membershipRole.createMany).toHaveBeenCalledWith({
      data: [
        { membershipId: "membership-1", roleId: "role-9", orgId: "org-1" },
      ],
    });
  });

  it("creates a membership when assigning a role to an existing user outside the org", async () => {
    ...
    expect(prismaMock.membershipRole.deleteMany).toHaveBeenCalledWith({
      where: { membershipId: "membership-2" },
    });
    expect(prismaMock.membershipRole.createMany).toHaveBeenCalledWith({
      data: [
        { membershipId: "membership-2", roleId: "role-9", orgId: "org-1" },
      ],
    });
  });
  ...
  it("rejects creating roles with permissions outside actor scope", async () => {
    ...
    prismaMock.permission.findMany = jest
      .fn()
      .mockResolvedValue([
        { id: "perm-1", name: "items.write", description: "" },
      ]);
  ...
  it("updates role permissions and description", async () => {
    ...
    prismaMock.role.update = jest.fn().mockResolvedValue({
      ...
      isSystem: false,
      permissions: [
        { permission: { id: "perm-1", name: "items.write", description: "" } },
      ],
    });

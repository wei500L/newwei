...
export class NewsAggregatorRegistryService implements OnModuleInit {
  ...
  private sources: Record<SourceID, Source> = Object.create(null)
  private getters: Record<SourceID, SourceGetter> = Object.create(null)
  ...
  getSource(id: SourceID): Source | undefined {
    if (!hasOwn(this.sources, id)) return undefined
  ...
  getGetter(id: SourceID): SourceGetter | undefined {
    if (!hasOwn(this.getters, id)) return undefined
  ...
  private buildGetters(): Record<SourceID, SourceGetter> {
    ...
    const getters: Record<string, SourceGetter> = Object.create(null)

    for (const [id, getterModule] of Object.entries(modules)) {
      if (FORBIDDEN_KEYS.has(id)) continue

      if (typeof getterModule === "function") {
        getters[id] = getterModule as SourceGetter
        continue
      }

      Object.entries(getterModule).forEach(([sourceId, getter]) => {
        if (FORBIDDEN_KEYS.has(sourceId)) return
        getters[sourceId] = getter as SourceGetter
      })
    }

...
import { UserSettingsService, createDefaultNewsnowUiSettings } from '../user-settings/user-settings.service';
...
export class NewsnowRecommendedService {
  ...
  async getRecommendedFeed(input: {
    ...
    const hottestSourceIds = (metadata.columns?.hottest?.sources ?? []).slice(0, 120);
    ...
    const [profile, collaborative, settingsResponse, analysisResult, batch] = await Promise.all([
      ...
      this.loadHottestAnalysis(input.orgId, input.userId, input.forceRefresh === true),
      this.aggregator.fetchBatch(hottestSourceIds, input.forceRefresh === true),
    ]);

    const settings = settingsResponse?.settings ?? createDefaultNewsnowUiSettings();
    ...
    const sourceResults = new Map(batch.results.map((result) => [result.id, result] as const));
    ...
    for (const sourceId of hottestSourceIds) {
      ...
      const items = Array.isArray(response.items) ? response.items.slice(0, MAX_ITEMS_PER_SOURCE) : [];
    ...
    const deduped = this.dedupeCandidates(candidates)
      ...
      .map((candidate) => ({
        ...
        ...(candidate.matchedItemId ? { matchedItemId: candidate.matchedItemId } : {}),
        ...(candidate.matchedEventId ? { matchedEventId: candidate.matchedEventId } : {}),
  ...
  private async loadHottestAnalysis(orgId: string, userId: string, forceRefresh: boolean) {
    try {
      return {
        degraded: false,
  ...
  private buildCandidate(input: {
    ...
    itemIndex: number;
    itemCount: number;
    analysisBySource: Record<string, {
      topics: string[];
      entities: string[];
    ...
    collaborative: Awaited<ReturnType<UserNewsBehaviorService['getCollaborativeProfile']>>;
    settings: ReturnType<typeof createDefaultNewsnowUiSettings>;
    maxContent: Record<'items' | 'events' | 'topics' | 'entities' | 'domains', number>;
    maxCollaborative: Record<'items' | 'events' | 'topics' | 'entities' | 'domains', number>;
    ...
    const contentScore = this.computePreferenceScore({
      ...
      matchedEventId: analyzed?.matchedEventId ?? null,
    ...
    return {
      ...
      url,
      mobileUrl:
        typeof input.item.mobileUrl === 'string' && input.item.mobileUrl.trim().length > 0
          ? input.item.mobileUrl.trim()
          : null,
      pubDate:
        typeof input.item.pubDate === 'number' || typeof input.item.pubDate === 'string'
          ? input.item.pubDate
          : null,
      domain,
      topics: analyzed?.topics ?? [],
  ...
  private buildDimensionMaxima(
    ...
    > | Pick<
      Awaited<ReturnType<UserNewsBehaviorService['getCollaborativeProfile']>>,
      'items' | 'events' | 'topics' | 'entities' | 'domains'
    >,
  ) {
    return {
  ...
  private computePreferenceScore(input: {
    ...
    maxima: Record<'items' | 'events' | 'topics' | 'entities' | 'domains', number>;
  }) {
  ...
  private computeSourceScore(
    ...
    const affinity = Number(Math.max(0, Math.min(1, affinityRaw / 100)).toFixed(4));
  ...
  private computeHotnessScore(
    ...
    if (
      ...
      return Number(Math.min(1, candidateScore * 0.7 + freshnessScore * 0.3).toFixed(4));
    }

...
jest.mock("@modular/mongo", () => ({
  DEFAULT_TASK_LOG_RETENTION_DAYS: 14,
  MIN_TASK_LOG_RETENTION_DAYS: 1,
  MAX_TASK_LOG_RETENTION_DAYS: 3650,
  TASK_LOG_TTL_INDEX_NAME: "task_log_created_at_ttl",
  TaskLogModel: {
...
describe("TaskLogSettingsService", () => {
  const prismaMock = {
    systemSetting: {
      findUnique: jest.fn(),
      upsert: jest.fn(),
  ...
  beforeEach(() => {
    ...
    prismaMock.systemSetting.upsert = jest
      .fn()
      .mockResolvedValue({ key: "task_log_settings" });
    prismaMock.systemSetting.deleteMany = jest
      .fn()
    ...
    service = new TaskLogSettingsService(prismaMock, envMock);
  });
  ...
  it("applies default TTL index on startup when no db setting exists", async () => {
    ...
    const ttlIndex = indexState.find(
      (index) => index.name === "task_log_created_at_ttl",
    );
    expect(ttlIndex?.expireAfterSeconds).toBe(14 * 24 * 60 * 60);
  });

  it("loads db retention settings and reconciles TTL after refresh", async () => {
    prismaMock.systemSetting.findUnique = jest
      .fn()
      .mockRejectedValueOnce(new Error("temporary outage"))
      .mockResolvedValueOnce({
        value: { retentionDays: 21 },
      });
    ...
    expect(ttlIndexAfterStartup?.expireAfterSeconds).toBe(14 * 24 * 60 * 60);
    ...
    expect(ttlIndexAfterRefresh?.expireAfterSeconds).toBe(21 * 24 * 60 * 60);
  });
  ...
  it("updates settings and synchronizes TTL index", async () => {
    setIndexState([
      { name: "_id_", key: { _id: 1 } },
      {
        name: "task_log_created_at_ttl",
        key: { createdAt: 1 },
        expireAfterSeconds: 14 * 24 * 60 * 60,
      },
    ]);

    const result = await service.updateSettings("org-1", "actor-1", 30);
  ...
  it("does not drop non-TTL createdAt indexes and fails fast with clear error", async () => {
    ...
    await expect(
      service.updateSettings("org-1", "actor-1", 45),
  ...
  it("retries transient index conflicts and succeeds", async () => {
    ...
    await service.updateSettings("org-1", "actor-1", 60);

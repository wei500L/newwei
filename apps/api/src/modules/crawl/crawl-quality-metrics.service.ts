...
export class CrawlQualityMetricsService {
  ...
  private async buildSnapshot(
    ...
    const [taskLogs, markdownDocs] = await Promise.all([
      TaskLogModel.find({
        orgId,
        queue: "crawl4ai",
        ...
        .select({ jobId: 1, stage: 1, status: 1, data: 1 })
        .lean(),
    ...
    const expansionLogs: typeof taskLogs = [];
    const preflightLogs: typeof taskLogs = [];
    const dedupeLogs: typeof taskLogs = [];
    for (const log of taskLogs) {
      if (log.stage === "expansion") {
        expansionLogs.push(log);
      } else if (log.stage === "preflight") {
        preflightLogs.push(log);
      } else if (log.stage === "dedupe") {
        dedupeLogs.push(log);
      }
    }
    ...
    for (const log of expansionLogs) {
      ...
      const data =
        log.data && typeof log.data === "object" && !Array.isArray(log.data)
          ...
          : {};
      const current = expansionByTask.get(taskId) ?? {
        triggered: false,
        improved: 0,
        lowSignal: false,
        candidateRejects: this.createEmptyRejectBreakdown(),
      ...
      const lowSignalResults =
        typeof data.lowSignalResults === "number" &&
        Number.isFinite(data.lowSignalResults)
      ...
      const confidenceBuckets = this.parseConfidenceBuckets(
        data.publishConfidenceBuckets,
      );
      ...
      const headSignal =
        ...
        !Array.isArray(data.headSignalEnrichment)
          ...
          : undefined;
      const attempted =
        typeof headSignal?.attempted === "number" &&
        Number.isFinite(headSignal.attempted)
          ? Math.max(0, Math.round(headSignal.attempted))
          : 0;
      const succeeded =
        typeof headSignal?.succeeded === "number" &&
        Number.isFinite(headSignal.succeeded)
      ...
      const inferredSoftFailureCount = headSignalSoftFailuresRecord
        ? this.toSafeNonNegativeInt(headSignalSoftFailuresRecord.httpStatus) +
          ...
          this.toSafeNonNegativeInt(
            headSignalSoftFailuresRecord.networkOrTimeout,
          ) +
      ...
      current.headSignalAttempted = Math.max(
        current.headSignalAttempted,
        attempted,
      );
      current.headSignalSucceeded = Math.max(
        current.headSignalSucceeded,
        succeeded,
      );
    ...
    const markdownByTask = new Map<
      string,
      { count: number; totalChars: number; emptyCount: number }
    >();
    for (const doc of markdownDocs) {
      ...
      const entry = markdownByTask.get(taskId) ?? {
        count: 0,
        totalChars: 0,
        emptyCount: 0,
      };
    ...
    const expansionTriggered = Array.from(expansionByTask.values()).filter(
      (entry) => entry.triggered,
    ).length;
    const expansionImproved = Array.from(expansionByTask.values()).filter(
      (entry) => entry.improved > 0,
    ).length;
    ...
    for (const entry of expansionByTask.values()) {
      ...
      candidateRejects.publishConfidence +=
        entry.candidateRejects.publishConfidence;
      publishConfidenceBuckets.lt04 += entry.publishConfidenceBuckets.lt04;
      publishConfidenceBuckets.from04To06 +=
        entry.publishConfidenceBuckets.from04To06;
      publishConfidenceBuckets.from06To08 +=
        entry.publishConfidenceBuckets.from06To08;
    ...
    const groupedBySource = this.groupBySource(
      tasks,
      expansionByTask,
      markdownByTask,
    );
  ...
  private groupBySource(
    ...
    for (const task of tasks) {
      const sourceId =
        this.extractSourceId(task.config, task.displayName) ?? "unknown";
      ...
      if (expansion) {
        current.candidateRejects.includePattern +=
          expansion.candidateRejects.includePattern;
        current.candidateRejects.excludePattern +=
          expansion.candidateRejects.excludePattern;
        current.candidateRejects.publishConfidence +=
          expansion.candidateRejects.publishConfidence;
        current.publishConfidenceBuckets.lt04 +=
          expansion.publishConfidenceBuckets.lt04;
        current.publishConfidenceBuckets.from04To06 +=
          expansion.publishConfidenceBuckets.from04To06;
        current.publishConfidenceBuckets.from06To08 +=
          expansion.publishConfidenceBuckets.from06To08;
        current.publishConfidenceBuckets.gte08 +=
          expansion.publishConfidenceBuckets.gte08;
        ...
        current.headSignalNoPublishSignal +=
          expansion.headSignalNoPublishSignal;
    ...
    return Array.from(bySource.entries())
      .map(([sourceId, value]) => ({
        ...
        expansionSuccessRate: this.safeRatio(
          value.expansionImproved,
          value.expansionTriggered,
        ),
        avgMarkdownChars:
          value.markdownCount > 0
            ? Math.round(value.markdownChars / value.markdownCount)
            : 0,
        candidateRejects: value.candidateRejects,
        publishConfidenceBuckets: value.publishConfidenceBuckets,
        fitMarkdownPreferenceRate: this.safeRatio(
          value.fitMarkdownCount,
          value.expansionTriggered,
        ),
        headSignalSuccessRate: this.safeRatio(
          value.headSignalSucceeded,
  ...
  private parseCandidateRejectBreakdown(
    value: unknown,
  ): CrawlQualityRejectBreakdown {
  ...
  private parseConfidenceBuckets(
    value: unknown,
  ): CrawlQualityConfidenceBuckets {
    if (!value || typeof value !== "object" || Array.isArray(value)) {

...
const makeMonitor = (overrides: Partial<Record<string, unknown>> = {}) => ({
  ...
  approvedSources: [] as string[],
  approvedGeos: [] as string[],
...
describe("SituationMonitorMonitorsService", () => {
  ...
  it("keeps semantic scores attached to the original candidate indexes after shortlist sorting", async () => {
    ...
    jest
      .spyOn(service as any, "loadMonitorRows")
      .mockResolvedValue([{ id: "row-1" }]);
    jest
      .spyOn(service as any, "normalizeMonitorRow")
      .mockReturnValue(makeMonitor());
    ...
    jest
      .spyOn(service as any, "tryRerankCandidates")
  ...
  it("matches canonical country geos from structured geo fields only", () => {
    const service = new SituationMonitorMonitorsService(
      {} as any,
      {} as any,
      {} as any,
    ...
    const result = (service as any).computeExplicitCandidateSignals(
      makeMonitor({ approvedGeos: ["JP"] }),
      makeCandidate({
        title: "Industrial policy update",
        summary: "No geo term in free text",
        geoTexts: ["Japan"],
      }),
    );
  ...
  it("does not treat short geo subscriptions as free-text substrings", () => {
    const service = new SituationMonitorMonitorsService(
      {} as any,
      {} as any,
      {} as any,
    ...
    const result = (service as any).computeExplicitCandidateSignals(
      makeMonitor({ approvedGeos: ["US"] }),
      makeCandidate({
        title: "Russia briefing",
        summary: "business coverage",
        geoTexts: ["Belarus"],
      }),
    );

...
describe("QueueGateway", () => {
  it("enforces max connections per user", async () => {
    const { QueueGateway } =
      require("./queue.gateway") as typeof import("./queue.gateway");
    ...
    expect(client2.emit).toHaveBeenCalledWith("queue:error", {
  ...
  it("rejects connection when rate limit exceeded", async () => {
    ...
    expect(rateLimiter.checkConnectionRateLimit).toHaveBeenCalledWith(
      "192.168.1.100",
    );
    expect(client.emit).toHaveBeenCalledWith("queue:error", {
  ...
  it("records failed auth and applies backoff", async () => {
    ...
    const authService = {
      getUserProfile: jest.fn().mockRejectedValue(new Error("User not found")),
    } as any;
    const accessTokenBlacklist = {
      has: jest.fn().mockResolvedValue(false),
    } as any;
  ...
  it("clears backoff on successful authentication", async () => {
    ...
    expect(client.emit).toHaveBeenCalledWith("queue:connected", {
      orgId: profile.orgId,
      userId: profile.id,
    });

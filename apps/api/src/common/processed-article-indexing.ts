...
export interface ProcessedArticleTermSource {
  title?: string | null;
  summary?: string | null;
  topics?: unknown;
}

export function normalizeProcessedArticleLocation(value: unknown): string | null {
  return normalizeString(value);
}

export function processedArticleHasLocation(value: unknown): boolean {
  return Boolean(normalizeProcessedArticleLocation(value));
}

export function resolveProcessedArticleEventAt(input: {
  publishedAt?: Date | null;
  crawlAt?: Date | null;
  processedAt?: Date | null;
}): Date {
  return input.publishedAt ?? input.crawlAt ?? input.processedAt ?? new Date();
}
...
export function extractProcessedArticleTerms(
  article: ProcessedArticleTermSource,
): string[] {

...
describe("news-source seed config mapping", () => {
  ...
  it("falls back to new recommended defaults for missing values", () => {
    ...
    expect(values.seedMaxNewUrlsPerRun).toBe(
      DEFAULT_SEED_FORM_VALUES.seedMaxNewUrlsPerRun,
    );
    expect(values.seedListMaxPages).toBe(
      DEFAULT_SEED_FORM_VALUES.seedListMaxPages,
    );
    expect(values.seedListPageConcurrency).toBe(
      DEFAULT_SEED_FORM_VALUES.seedListPageConcurrency,
    );
    expect(values.seedFollowPagination).toBe(
  ...
  it("uses mode-aware cache TTL defaults when cacheTtlSeconds is missing", () => {
    ...
    expect(listValues.seedCacheTtlSeconds).toBe(
      getDefaultSeedCacheTtlSecondsByMode("list"),
    );
    expect(rssValues.seedCacheTtlSeconds).toBe(
      getDefaultSeedCacheTtlSecondsByMode("rss"),
    );
  });
  ...
  it("resolves effective cache ttl with source override when global force is off", () => {
    const resolved = resolveSeedCacheTtlPolicy("sitemap", 900, {
      ...DEFAULT_SEED_SCHEDULER_RUNTIME_SETTINGS,
      seedCacheTtlForceGlobal: false,
    });
  ...
  it("resolves effective cache ttl with global defaults when force is on", () => {
    const resolved = resolveSeedCacheTtlPolicy("deep", 900, {
  ...
  it("reads rss advanced fetch settings from config", () => {
    const values = readSeedFormValuesFromConfig({
      seed: {
        enabled: true,
        mode: "rss",
    ...
    expect(values.seedRssAdvancedEnabled).toBe(true);
    expect(values.seedRssRequestTimeoutMs).toBe(20000);
    expect(values.seedRssBodySourceStrategy).toBe("summary_only");
    expect(values.seedRssNoBodyPolicy).toBe("title_description_stub");
  });

  it("builds rss seed config with advanced fetch settings", () => {
    const next = buildSeedConfigFromFormValues(
      {
        seedEnabled: true,
        seedMode: "rss",
        seedFeedUrl: "https://example.com/rss.xml",
        seedRssAdaptiveEnabled: true,
        seedRssAdvancedEnabled: true,
        seedRssRequestTimeoutMs: 20000,
        seedRssBodySourceStrategy: "summary_only",
        seedRssNoBodyPolicy: "title_description_stub",
      },
      null,
    );
  ...
  it("hides crawl settings only for active rss seed mode", () => {
    expect(shouldShowCrawlSettingsForSeedMode(true, "rss")).toBe(false);
    expect(shouldShowCrawlSettingsForSeedMode(false, "rss")).toBe(true);
    expect(shouldShowCrawlSettingsForSeedMode(true, "list")).toBe(true);
  });
  ...
  it("resolves adaptive interval and discovery ttl by tier", () => {
    ...
    expect(resolveSeedRssAdaptiveIntervalSeconds(600, "warm", runtime)).toBe(
      200,
    );
    expect(resolveSeedRssAdaptiveIntervalSeconds(600, "cold", runtime)).toBe(
      600,
    );
    expect(resolveSeedRssAdaptiveIntervalSeconds(120, "cold", runtime)).toBe(
      240,
    );
    expect(
      resolveSeedRssAdaptiveDiscoveryCacheTtlSeconds(180, "hot", runtime),
    ).toBe(20);
    expect(
      resolveSeedRssAdaptiveDiscoveryCacheTtlSeconds(180, "warm", runtime),
    ).toBe(50);
    expect(
      resolveSeedRssAdaptiveDiscoveryCacheTtlSeconds(180, "normal", runtime),
    ).toBe(180);
  });
});

...
describe("faceted-search state", () => {
  it("defaults layered expansion to groups with selected values only", () => {
    expect(
      resolveFacetedDefaultActiveKeys({
        behavior: "layered",
        availableKeys: ["region", "topic", "sentiment", "contentType"],
        filters: {
          regions: ["APAC"],
          sentiments: ["positive"],
          contentTypes: ["news_fact"],
        },
      }),
    ).toEqual(["region", "sentiment", "contentType"]);
  });
  ...
  it("computes selected summary counts including date range", () => {
    expect(
      getFacetedSelectionCounts({
        ...
        topics: ["AI", "Finance"],
        sentiments: ["neutral"],
        contentTypes: ["analysis", "news_fact"],
  ...
  it("filters section options case-insensitively", () => {
    expect(
      filterFacetOptions(["North America", "Asia Pacific", "Europe"], "ASIA"),
    ).toEqual(["Asia Pacific"]);
  });
});

...
const ECONOMIC_WIDGET_QUERY = gql`
  query EconomicWidgetData($category: String!, $timeRange: DateRangeInput!, $granularity: TimeGranularity) {
...
const parseDataSource = (dataSource: string) => {
  if (dataSource.startsWith("economic:")) {
    return { kind: "economic" as const, category: dataSource.replace("economic:", "") };
  }
  return { kind: "unknown" as const };
};

const chooseGranularity = (start: Date, end: Date, explicit?: any) => {
...
export function WidgetRenderer({ type, title, data, dataSource, color }: WidgetRenderProps) {
  ...
  const resolvedData =
    data ??
    apiData?.getEconomicData?.map((p: any) => ({
  ...
  const option = useMemo(() => {
    ...
    switch (type) {
      case "bar":
        return {
          ...common,
          series: [{ type: "bar", data: seriesData.map(([, v]) => v), barWidth: "60%" }],
          xAxis: { type: "category", data: resolvedData?.map((p) => p.timestamp) }
        };
      case "pie":
        return {
          ...common,
          series: [
            { type: "pie", radius: "60%", data: resolvedData?.map((p) => ({ name: p.timestamp, value: p.value })) ?? [] }
          ]
        };
      case "scatter":
        return {
      ...
      case "radar":
        return {
          ...
          series: [
            { type: "radar", data: [{ value: resolvedData?.map((p) => p.value) ?? [], name: title ?? dataSource }] }
          ]
        };
      default:
        return {
  ...
  if (sourceInfo.kind === "unknown") {
    return <Typography.Text type="secondary">Unsupported data source: {dataSource}</Typography.Text>;
  }
  ...
  const theme = color ? { color: [color], textStyle: { color: "#333" }, title: { textStyle: { color } } } : undefined;
  if (theme) {
    const themeName = `custom-${color}`;
    echarts.registerTheme(themeName, theme);
    return <DashboardChart option={option} height={300} group="linked-charts" renderer="canvas" />;
  }

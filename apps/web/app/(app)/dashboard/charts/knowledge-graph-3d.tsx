...
export function KnowledgeGraph3D({ defaultSeed }: KnowledgeGraph3DProps) {
  ...
  const { pending: refreshingSettings, run: refreshSettings } =
    usePendingAction(() => refetchSettings());
  ...
  useEffect(() => {
    ...
    try {
      const saved = window.localStorage
        .getItem(LAST_ENTITY_STORAGE_KEY)
        ?.trim();
      if (!saved) {
        return;
      }
  ...
  const { data, loading, error, refetch } = useGetKnowledgeGraphSubgraphQuery({
    ...
    skip:
      !authenticated || !canReadDashboards || isDisabledByAdmin || !seedName,
  });
  ...
  const degreeMap = useMemo(
    () => (graph ? buildDegreeMap(graph.edges) : new Map<string, number>()),
    [graph],
  );
  ...
  useEffect(() => {
    ...
    const setup = async () => {
      ...
      if (cancelled || containerRef.current !== dom) {
        return;
      }
      ...
      for (const node of nodes) {
        ...
        const material = new THREE.MeshStandardMaterial({
          ...
          emissive: new THREE.Color(
            isSeed ? (colors?.primary ?? "#1f3b7b") : "#000000",
          ),
      ...
      const applyForces = (dt: number) => {
        ...
        for (let i = 0; i < count; i += 1) {
          ...
          velocities[ix + 1] =
            (velocities[ix + 1]! + forces[ix + 1]! * dt) * damping;
          velocities[ix + 2] =
            (velocities[ix + 2]! + forces[ix + 2]! * dt) * damping;
          ...
          nodeObjects[i]!.mesh.position.set(
            positions[ix]!,
            positions[ix + 1]!,
            positions[ix + 2]!,
          );
        ...
        (
          edgeGeometry.attributes.position as THREE.BufferAttribute
        ).needsUpdate = true;
        return maxVelocity;
      };
      ...
      const handlePointerDown = (evt: PointerEvent) => {
        ...
        const hit = intersections[0]?.object as
      ...
      function requestRender(interactionWindowMs = 0) {
        ...
        interactiveUntilMs = Math.max(
          interactiveUntilMs,
          now + interactionWindowMs,
        );
        ...
        <Space size="small" wrap style={{ marginBottom: "0.75rem" }}>
          ...
          {settings.dynamicEdgeConfidenceEnabled ? (
            <Tag>
              ...
              Q
              {Number(settings.dynamicEdgeConfidenceQuantile ?? 0.25).toFixed(
                2,
              )}
            </Tag>
          ) : null}
          {settings.multiModelValidationEnabled ? (
            <Tag color="green">
              {t("settings.knowledgeGraph.fields.multiModelValidationEnabled")}
            </Tag>
          ) : null}
          {settings.entityDisambiguationEnabled ? (
            <Tag color="geekblue">
              {t("settings.knowledgeGraph.fields.entityDisambiguationEnabled")}
            </Tag>
          ) : null}
        </Space>
      ) : null}
      ...
      {!seedName ? (
        ...
        <div className="h-[380px]">
          <ChartEmptyState
            ...
            description={t(
              "dashboard.charts.knowledgeGraphNotFoundDescription",
            )}
          />
        </div>
      )}

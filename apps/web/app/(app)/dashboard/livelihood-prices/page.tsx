...
export default function LivelihoodPricesPage() {
  const { loading, error, seriesMap } = useEconomicData({ category: "livelihood-prices", pollInterval: 180_000 });

  const cpiTreeData = [
    {
      name: "食品 (全国环比)",
      value: getLatestValue(getSeriesField(seriesMap, "china_cpi", "全国-环比增长"))?.value ?? 0
    },
    {
      name: "居住 (城市环比)",
      value: getLatestValue(getSeriesField(seriesMap, "china_cpi", "城市-环比增长"))?.value ?? 0
    },
    {
      name: "交通 (农村环比)",
      value: getLatestValue(getSeriesField(seriesMap, "china_cpi", "农村-环比增长"))?.value ?? 0
    }
  ];
  ...
  const treeOption = {
    ...
    series: [
      {
        type: "treemap",
        data: cpiTreeData.map((item) => ({ ...item, value: Number(item.value.toFixed(2)) }))
      }
    ]
  };

  const radarData = agConfigs.map((config) => {
    const latest = getLatestValue(getSeriesField(seriesMap, config.slug, "收盘价"))?.value ?? 0;
  ...
  const tourismSeries = getSeriesField(seriesMap, "china_international_tourism_fx", "数量");
  const tourismRatioSeries = getSeriesField(seriesMap, "china_international_tourism_fx", "比重");
  const tourismValues = getSortedValues(tourismSeries);
  const ratioValues = new Map(getSortedValues(tourismRatioSeries).map((entry) => [entry.timestamp, entry.value]));
  const tourismOption = {
    ...
    series: [
      ...
      },
      {
        ...
        type: "line",
        yAxisIndex: 1,
        data: tourismValues.map((entry) => [entry.timestamp, ratioValues.get(entry.timestamp) ?? null])
      }
    ]
  };

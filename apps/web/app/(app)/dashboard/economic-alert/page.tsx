...
import { getLatestValue, getSeriesField, getSortedValues } from "../utils/series";

export default function EconomicAlertPage() {
  const { loading, error, seriesMap } = useEconomicData({ category: "economic-alert", pollInterval: 60_000 });
  ...
  const usPmiSeries = getSeriesField(seriesMap, "us_services_pmi", "current_value");
  const usCpiSeries = getSeriesField(seriesMap, "us_cpi_monthly", "今值");
  const usPpiSeries = getSeriesField(seriesMap, "us_ppi_monthly", "今值");
  const usManufacturingPmiSeries = getSeriesField(seriesMap, "us_manufacturing_pmi", "今值");

  const tenYearSeries = getSeriesField(seriesMap, "us_treasury_yield_curve", "美国国债收益率10年");
  const twoYearSeries = getSeriesField(seriesMap, "us_treasury_yield_curve", "美国国债收益率2年");
  const twoYearMap = new Map(getSortedValues(twoYearSeries).map((point) => [point.timestamp, point.value]));
  const spreadSeries = getSortedValues(tenYearSeries)
    .map((point) => {
      ...
      return { timestamp: point.timestamp, value: Number((point.value - short).toFixed(3)) };
    })
  ...
  if (getLatestValue(chinaPmiSeries)?.value !== undefined && (getLatestValue(chinaPmiSeries)?.value ?? 0) < 50) {
    ...
    series: [
      {
        name: "中国PMI",
        type: "line",
        data: getSortedValues(chinaPmiSeries).map((point) => [point.timestamp, point.value]),
        smooth: true
      },
      {
        name: "美国PMI",
        type: "line",
        data: getSortedValues(usPmiSeries).map((point) => [point.timestamp, point.value]),
        smooth: true
      },
      {
        type: "line",
        name: "荣枯线",
        data: getSortedValues(chinaPmiSeries).slice(-50).map((entry) => [entry.timestamp, 50]),
    ...
    series: [
      {
        ...
        type: "line",
        smooth: true,
        data: getSortedValues(usCpiSeries).map((point) => [point.timestamp, point.value])
      },
      {
        name: "美国PPI",
        type: "line",
        smooth: true,
        data: getSortedValues(usPpiSeries).map((point) => [point.timestamp, point.value])
      }
    ]
  };
    ...
    series: [
      {
        ...
        type: "line",
        smooth: true,
        data: getSortedValues(usManufacturingPmiSeries).map((point) => [point.timestamp, point.value])
      },
      {
        name: "服务业PMI",
        type: "line",
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      {error && <Alert type="error" message="经济数据加载失败" description={error.message} showIcon />}
      {alerts.length > 0 && (
        <Alert
          type="warning"
          showIcon

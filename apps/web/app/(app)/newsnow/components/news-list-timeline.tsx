...
export function NewsListTimeline({ items }: NewsListTimelineProps) {
  ...
  return (
    <ul className="space-y-1 py-1">
      {items.map((item) => (
        <li key={`${item.id}-${item.pubDate || item.extra?.date || ""}`} className="group flex items-start gap-4 rounded-md px-2 py-1.5 transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-800">

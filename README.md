...
- `Operations -> News Sources` 支持 **OPML 批量导入 RSS/Atom 源**：`Import OPML` 会将 OPML 的 `htmlUrl` 作为 `NewsSource.url`，并把 `xmlUrl` 写入 `config.seed.feedUrl`（Seed mode = RSS）。每个源可指定 `language`（例如 `zh`/`en`），下游新闻清洗链路会用该语言提示 LLM 输出对应语言内容，从而实现“翻译走新闻链路”。重复 URL 会被跳过并在导入报告中列出。

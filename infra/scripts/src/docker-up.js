...
const log = (message) =>
  process.stdout.write(`${nowIso()} [docker-up] ${message}\n`);
const logError = (message) =>
  process.stderr.write(`${nowIso()} [docker-up] ${message}\n`);
...
const isFlagWithValue = (flag, arg) =>
...
const collectRedisAofDiagnostics = (composeBaseArgs, cwd) => {
  ...
  for (const diagnostic of diagnostics) {
    ...
    if (result.error) {
      logError(
        `Failed to execute diagnostic command: ${summarizeCommand("docker", diagnostic.args)}`,
      );
      logError(`           ${result.error.message}`);
      continue;
    }
...
const ensurePnpmLockfileForDockerUp = (repoRoot) => {
  ...
  const lockfileCheckWorkspaceDir = path.resolve(
    repoRoot,
    ".cache/docker-up/pnpm-lockfile-check",
  );
  ...
  try {
    ...
    if (checkResult.error) {
      if (checkResult.error.code === "ETIMEDOUT") {
        logError(
          "pnpm lockfile preflight timed out after 120s (likely waiting on an interactive prompt or lock).",
        );
        logError(
          `           Try manually: ${summarizeCommand("pnpm", checkArgs)}`,
        );
      }
      logError(
        `Failed to execute command: ${summarizeCommand("pnpm", checkArgs)}`,
      );
    ...
    const isOutdatedLockfile = captureOutput(checkResult).includes(
    ...
    if (syncResult.error) {
      if (syncResult.error.code === "ETIMEDOUT") {
        logError(
          "pnpm lockfile synchronization timed out after 120s (likely waiting on an interactive prompt or lock).",
        );
        logError(
          `           Try manually: ${summarizeCommand("pnpm", syncArgs)}`,
        );
      }
      logError(
        `Failed to execute command: ${summarizeCommand("pnpm", syncArgs)}`,
      );
...
const resolveUpServices = (upArgs, services) => {
  ...
  const explicit = upArgs.filter(
    (arg) => arg && !arg.startsWith("-") && serviceNames.has(arg),
  );
...
const resolvePrismaCliPackageForDockerUp = (repoRoot) => {
  ...
  try {
    ...
    for (const line of lockfileLines) {
      ...
      if (
        typeof prismaVersionMatch?.[1] === "string" &&
        prismaVersionMatch[1].trim().length > 0
      ) {
...
const resolveMysqlConnectionStringForDockerUp = (envFile) => {
  const fileEnv = existsSync(envFile)
    ? parseDotenv(readFileSync(envFile, "utf8"))
    : {};
  const mergedEnv = { ...fileEnv, ...process.env };
  const databaseUrl =
    typeof mergedEnv.DATABASE_URL === "string"
      ? mergedEnv.DATABASE_URL.trim()
      : "";
  ...
  if (missingKeys.length > 0) {
    throw new Error(
      `Missing required MySQL env vars in ${envFile}: ${missingKeys.join(", ")}`,
    );
  }

  return `mysql://${mergedEnv.MYSQL_USER}:${encodeURIComponent(mergedEnv.MYSQL_PASSWORD)}@${mergedEnv.MYSQL_HOST}:${mergedEnv.MYSQL_PORT}/${mergedEnv.MYSQL_DB}`;
};

const validatePrismaSchemaForDockerUp = (composeBaseArgs, cwd, envFile) => {
  const repoRoot = path.resolve(cwd, "../..");
  const prismaCliPackage = resolvePrismaCliPackageForDockerUp(repoRoot);
  const databaseUrl = resolveMysqlConnectionStringForDockerUp(envFile);
  const checkScript = [
    "set -eu",
    `export DATABASE_URL='${databaseUrl.replace(/'/g, "'\\''")}'`,
  ...
  logError(
    "Prisma schema validation failed. Fix packages/db/prisma/schema.prisma and retry.",
  );
  process.exit(result.status ?? 1);
};
...
const main = () => {
  ...
  const wantsExtrasProfile = requestedServices.some((name) =>
    knownExtrasServices.has(name),
  );
  ...
  if (upServices.has("api")) {
    ...
    if (!dockerImageExists(apiImageRef, scriptsDir)) {
      log(
        "API image is missing; building api image for Prisma schema preflight...",
      );
  ...
  if (upServices.has("redis")) {
    const runningServices = listRunningComposeServices(
      composeBaseArgs,
      scriptsDir,
    );
  ...
  const buildServices = Object.entries(services)
    .filter(
      ([name, service]) =>
        upServices.has(name) && Boolean(service && service.build),
    )
    .map(([name]) => name);
  ...
  const pullServices = Object.entries(services)
    .filter(
      ([name, service]) =>
        upServices.has(name) && typeof (service && service.image) === "string",
    )
    .map(([name]) => name);
  ...
  if (missingPullImages.length > 0) {
    run(
      "docker",
      [...composeBaseArgs, "pull", ...missingPullImages],
      scriptsDir,
    );
  }
  ...
  if (missingBuildImages.length > 0) {
    run(
      "docker",
      [...composeBaseArgs, "build", ...missingBuildImages],
      scriptsDir,
    );
  }
